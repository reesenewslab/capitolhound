...
/* numbers only for transcript_id */
$search_transcript_id = 0;
$search_transcript_id = preg_replace('/[^0-9]/', '', $_GET['transcript_id']);
...
if ($search_transcript_id > 0) {
          echo "dsbjfbsfbsdfjbsjdbf";
          $title_query = "SELECT title, date, b.chamber, b.year, b.location, b.description FROM transcript b
                          WHERE transcript_id = $search_transcript_id";
          $title_result = $mysqli->query($title_query);

          $title_obj = $title_result->fetch_object();
          /* free up the mysql results (although it's really unnecessary here) */
          $title_result->free();

          $transcript_title_display = date("F n, Y", strtotime($title_obj->date)) . " | " . $title_obj->title;

          print "<h3>Transcript: " . $transcript_title_display . " </h3>";
}
else {
...
<?php
   ...
   /* $human_start_date = date("F n, Y", strtotime($start_date));;
    $human_end_date = strtotime($end_date);

    //print "<h3>Searching between dates " . date("F n, Y ", strtotime($start_date)) . " and " . date("F n, Y", strtotime($end_date)) . "</h3>";
    //print "<h3>Searching between dates " . $human_start_date . " and " . $human_end_date . "</h3>";
	...
	$jsonStr = json_decode(file_get_contents($api), true);
...
echo '<div class="col-sm-12">';
		...
		echo '<tr><th>FullName</th><th>Email</th><th>Country</th><th>State</th><th>District</th><th>Party</th><th>Chamber</th><th>Level</th><th>Offices</th></tr>';*/
/*foreach($jsonStr as $key => $val){
if($val["last_name"] == $search_term)

 
 

{
echo '<h4><b>FullName : </b>'. $val['full_name'] . '<br> </h4>' ;
 echo '<h4><b>Email-Id : </b>'. $val['email'] . '<br> </h4>' ;
 echo '<h4><b>Country : </b>'. $val['country'] . '<br> </h4>' ;
 echo '<h4><b>State : </b>'. $val['state'] . '<br> </h4>' ;
 echo '<h4><b>District : </b>'. $val['district'] . '<br> </h4>' ;
 echo '<h4><b>Party : </b>'. $val['party'] . '<br> </h4>' ;
 echo '<h4><b>Chamber : </b>'. $val['chamber'] . '<br> </h4>' ;
 echo '<h4><b>Level : </b>'. $val['level'] . '<br> </h4><br>' ;*/
//var_dump($val);
/*echo "<tr>";
			echo "<td>";
			echo $val["full_name"];
	...
	/*foreach($val['offices'] as $key1 => $val1){
	
	echo '<h4><b>Office Name : </b>'. $val1['name'] . '<br> </h4>' ;
	echo '<h4><b>Office Contact Number : </b>'. $val1['phone'] . '<br> </h4>' ;
	echo '<h4><b>Office Address : </b>'. $val1['address'] . '<br> </h4>' ;
	echo '<h4><b>Office Type : </b>'. $val1['type'] . '<br> </h4><br> ' ;
	/*echo "<td>";
			echo $val1['name']; echo "<br>";
	echo $val1['phone']; echo "<br>";
	echo $val1['address']; echo "<br>";
	echo $val1['type']; echo "<br>";
			echo "</td>";*/
	...
	//}
	//echo "</tr>";
	//echo $val['address']; echo "<br>";//address
  // }
//}
//echo '</table>';
	//	echo '</div>';*/
...
$s->SetLimits(0, 100000);
$s->setFilterRange(timestamp, strtotime($start_date), strtotime($end_date));

if ($search_transcript_id > 0) {
    //$s->setFilterRange(transcript_id, (int)$search_transcript_id, (int)$search_transcript_id);
    $s->setFilter(transcript_id, array($search_transcript_id));
}

$result = $s->query($search_term);

//$temp = count($result);
//print "Results found: $temp<br>";
...
//print "COUNT is $result_count<br>";
...
/* loop through the query results and store everything in a variable to make things a bit easier */
    while($obj = $result->fetch_object()) {
        ...
        */
        ...
        $transcript_temp = str_ireplace($search_term, $search_term, $transcript, $count);
