...
$step = preg_replace('/[^0-9]/', '', $_REQUEST['step']);

/* if step paramter is not passed, then give them the default search form */
if (! $step) {
    require_once("models/search_demo.php");
    print "<p style='display:block; text-align: center; font-size: 85%;'>The dates in this demo are preset to May 14, 2014 to May 21, 2014 and cannot be changed</p>";
}
/* Do the actual searching */
else {

/* numbers only for page_number  */
$page_number = preg_replace('/[^0-9]/', '', $_REQUEST['page_number']);

/* numbers and "-" only for dates */
$start_date = preg_replace('/[^0-9]-/', '', $_REQUEST['2014-05-14']);
$end_date = preg_replace('/[^0-9]-/', '', $_REQUEST['2014-05-21']);

/* a little more allowed for search term */
$search_term = preg_replace('/[^-a-zA-Z0-9\'  ]/', '', $_REQUEST['search_term']);

/* if not date is specified then we want it all - set a wide date range */
/*if (! $start_date) {*/
...
print "<h1 class='sad'>Searching for $uppercase_search_term</h1>";
...
/* loop through the query results and store everything in a variable to make things a bit easier */
    while($obj = $result->fetch_object()) {
        ...
        */
        $excerpt_text = create_search_results_excerpt($transcript, $uppercase_search_term);
